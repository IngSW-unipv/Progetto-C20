...
public class Menu extends MouseAdapter{
	...
	public void mousePressed(MouseEvent e){
		...
		if(game.getState() == State.End){
			...
			if(mouseOver(mx, my, (game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (0*y), 200, 64)){
				
				//create level con 1 player
				this.game.setLevel(1);
				this.game.setState(State.Game);
				
			}else if(mouseOver(mx, my, (game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (1*y), 200, 64)){
				
				// 2
				this.game.setLevel(2);
				this.game.setState(State.Game);
				
			}else if(mouseOver(mx, my, (game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (2*y), 200, 64)){
				
				//3
				this.game.setLevel(3);
				this.game.setState(State.Game);
				
			}else if(mouseOver(mx, my, (game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (3*y), 200, 64)){
	...
	public void render(Graphics g) {
		...
		if(game.getState()  == State.End){
			...
			g.drawRect((game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (0*y), x, y);
			g.drawString("1", (game.getWIDTH() / 2) - w2, h2+(0*y)+((game.getHEIGHT() - (4* y))/2));
			
			w2 = g.getFontMetrics().stringWidth("2") / 2;
			g.drawRect((game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2)+(1*y), x, y);
			g.drawString("2", (game.getWIDTH() / 2) - w2, h2+(1*y)+((game.getHEIGHT() - (4* y))/2));
			
			w2 = g.getFontMetrics().stringWidth("3") / 2;
			g.drawRect((game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2)+(2*y), x, y);
			g.drawString("3", (game.getWIDTH() / 2) - w2, h2+(2*y)+((game.getHEIGHT() - (4* y))/2));
			
			w2 = g.getFontMetrics().stringWidth("4") / 2;
			g.drawRect((game.getWIDTH()/2)-(x/2), ((game.getHEIGHT() - (4* y))/2) + (3*y), x, y);
			g.drawString("4", (game.getWIDTH() / 2) - w2, h2+(3*y)+((game.getHEIGHT() - (4* y))/2));
